...
def model_2(last_iteration=17):
    ...
    ahf.Casualty_test(df2,Model_res).to_csv('Output/Model 2/with GDP def/CausaltyTestResults.csv')
    ...
    ahf.AutoCorrtest(Model_res,10).to_csv('Output/Model 2/with GDP def/AutocorreleationTestResults.csv')
    ...
    forecasts_trans_df = forecast_extract.head(17).copy()
    ...
    out.to_csv('Output/Model 2/with GDP def/ActualforecastModel2.csv')
    ...
    for column in out.columns:
        ...
        fig = ax.get_figure()
        fig.savefig('Output/Variations/Variant 1/Model 2 GDPD '+column+'.png')
        pd.DataFrame(temp_dict,index=oe_forecasts_df.index).to_csv(
            'Output/Model 2/with GDP def/Adj Forecasts/'+column+\
...
################## Model CPI 2 below
def model_CPI_2(last_iteration=17):
    ...
    #Granger Causalty test
    ahf.Casualty_test(df2,Model_res).to_csv('Output/Model 2/with CPI/CausaltyTestResults.csv')
    ...
    #Autocorreleation test
    ahf.AutoCorrtest(Model_res,10).to_csv('Output/Model 2/with CPI/AutocorreleationTestResults.csv')
    ...
    for lg in range(len(log_list)):
        if log_list[lg]==1:
            out.iloc[:,lg] = np.exp(out.iloc[:,lg])
    out.to_csv('Output/Model 2/with CPI/ActualforecastModel2.csv')
    ...
    final_output_Model2 = out
    out.to_csv('Output/Model 2/with CPI/Model2AdjOutput.csv')
    new_adj_final_forecasts = out.copy()
    ...
    for column in out.columns:
        ...
        #ax.set_ylim(0, np.array(pd.DataFrame(temp_dict,index=oe_forecasts_df.index)).max()*1.5)
        fig = ax.get_figure()
        fig.savefig('Output/Variations/Variant 1/Model 2 CPI '+column+'.png')
        pd.DataFrame(temp_dict,index=oe_forecasts_df.index).to_csv(
            'Output/Model 2/with CPI/Adj Forecasts/'+column+\
                '_adj_forecasts_model_2.csv')
        pd.DataFrame(temp_dict,index=oe_forecasts_df.index).to_csv(
            'Output/Variations/Variant 1/Adj Forecasts/Model 2/'+column+\
                '_adj_forecasts_model_2.csv')
...
def model_HW_2(last_iteration=14):
    ...
    df['lCREPriceIndex'] = np.log(df['CREPriceIndex'])
    df['lNominalMortgageDebt'] = np.log(df['NominalMortgageDebt'])
    df['30YFixedMortgageRate'] = np.log(df['30YFixedMortgageRate'])
    df['lHousingStarts'] = np.log(df['HousingStarts'])
    df2 = df.drop(['Consumption', 'CPI', 'ResidentialInvestment', 'CREPriceIndex','NominalMortgageDebt', '3M_InterestRate','HousingStarts'], axis = 1)
    df2 = df2[['lConsumption','lNominalMortgageDebt','lCREPriceIndex','lCPI','30YFixedMortgageRate','lResidentialInvestment','lHousingStarts']]
    df2.columns
    #   
    #With constant "0", lagorder "2"
    mod1 = coint_johansen(df2,0,1)
    ...
    for i in range(14):
        forecast_extract = forecast_extract.append({'lConsumption':Model_res.predict(steps = len(exog), exog_fc = exog)[i][0],
                                                    ...
                                                    'lCPI':Model_res.predict(steps = len(exog), exog_fc = exog)[i][3],
                                                    '30YFixedMortgageRate':Model_res.predict(steps = len(exog), exog_fc = exog)[i][4],
                                                    'lResidentialInvestment':Model_res.predict(steps = len(exog), exog_fc = exog)[i][5],
                                                    'lHousingStarts':Model_res.predict(steps = len(exog), exog_fc = exog)[i][6]},ignore_index=True)
     
    forecasts_trans_df = forecast_extract.head(14).copy()
    forecasts_trans_df = forecasts_trans_df[['lConsumption','lNominalMortgageDebt','lCREPriceIndex','lCPI','30YFixedMortgageRate','lResidentialInvestment','lHousingStarts']]
    out = forecasts_trans_df.copy()
    ...
    lag_order = 3
    upstream_forecasts = pd.read_csv(
        'Input/Model 2/Model 1 with HW Transformed Output for Model 2.csv',
        index_col=0
        )
   ...
   # upstream_forecasts = pd.read_csv(
    ...
    new_forecasts_df_mod2.columns = ['lConsumption','lNominalMortgageDebt','lCREPriceIndex','lCPI','30YFixedMortgageRate','lResidentialInvestment','lHousingStarts']
...
def model_HW_2_cpi(last_iteration=14):
    ...
    df['lNominalMortgageDebt'] = np.log(df['NominalMortgageDebt'])
    df['30YFixedMortgageRate'] = np.log(df['30YFixedMortgageRate'])
    df2 = df.drop(['Consumption', 'ResidentialInvestment', 'CREPriceIndex','NominalMortgageDebt', '3M_InterestRate'], axis = 1)
    df2 = df2[['lConsumption','lNominalMortgageDebt','lCREPriceIndex','30YFixedMortgageRate','lResidentialInvestment']]
    df2.columns
    #   
    #With constant "0", lagorder "2"
    mod1 = coint_johansen(df2,0,1)
    ...
    forecasts_trans_df = forecasts_trans_df[['lConsumption','lNominalMortgageDebt','lCREPriceIndex','30YFixedMortgageRate','lResidentialInvestment']]
    out = forecasts_trans_df.copy()
   ...
   # upstream_forecasts = pd.read_csv(
    ...
    new_forecasts_df_mod2.columns = ['lConsumption','lNominalMortgageDebt','lCREPriceIndex','30YFixedMortgageRate','lResidentialInvestment']
